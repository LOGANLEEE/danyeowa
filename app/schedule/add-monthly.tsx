...
export default function AddMonthlyScreen() {
  ...
  const [numpadFlightId, setNumpadFlightId] = useState<string | null>(null);
  ...
  const handleNumpadSubmit = () => {
    if (!numpadFlightId || !numpadTarget) return;

    const flight = flights.find((f) => f.id === numpadFlightId);
    if (!flight) return;

    if (numpadTarget === 'flightNumber') {
      updateFlightField(flight.id, 'flightNumber', numpadValue);
    } else if (numpadTarget === 'returnFlightNumber') {
      updateFlightField(flight.id, 'returnFlightNumber', numpadValue);
    }

    setNumpadFlightId(null);
    setNumpadValue('');
    setNumpadTarget(null);
  };

  const openNumpad = (
    target: 'flightNumber' | 'returnFlightNumber',
    flightId: string,
  ) => {
    const flight = flights.find((f) => f.id === flightId);
    if (!flight) return;
    setNumpadFlightId(flightId);
    setNumpadTarget(target);
    setNumpadValue(
      target === 'flightNumber' ? flight.flightNumber : flight.returnFlightNumber || '',
    );
  };
        ...
        <View className="mb-6">
          ...
          {flights.length === 0 ? (
            <EmptyState
              ...
              iconSize={40}
              message='No flights added yet. Tap calendar dates or use "Add Row" to get started.'
            />
          ) : (
                      ...
                      <View className="gap-3 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <View>
                          ...
                          <View className="flex-row gap-3">
                            <TouchableOpacity
                              onPress={() => openNumpad('flightNumber', flight.id)}
                              className={`flex-1 px-4 py-3 rounded-lg bg-white dark:bg-gray-800 border-2 ${
                                numpadFlightId === flight.id && numpadTarget === 'flightNumber'
                                  ? 'border-[#800020] dark:border-[#A0002A]'
                        ...
                        <View>
                          ...
                          <FlightTypeToggle
                            ...
                            onChange={(value) => updateFlightField(flight.id, 'flightType', value)}
                          />
                        </View>
                          ...
                          <View className="mt-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                            ...
                            {flight.returnDate ? (
                              <View className="gap-3">
                                ...
                                <View className="flex-row gap-3">
                                  <TouchableOpacity
                                    onPress={() => openNumpad('returnFlightNumber', flight.id)}
                                    className={`flex-1 px-4 py-3 rounded-lg bg-white dark:bg-gray-800 border-2 ${
                                      numpadFlightId === flight.id &&
                                      numpadTarget === 'returnFlightNumber'
                                        ? 'border-[#800020] dark:border-[#A0002A]'
                                        : 'border-gray-300 dark:border-gray-600'
                                    }`}>
                                    <ThemedText className="text-lg font-semibold">
                                ...
                                <View className="flex-row gap-3">
                                  <View className="flex-1">
                                    ...
                                    <ThemedInput
                                      ...
                                      onChangeText={(text) =>
                                        updateFlightField(flight.id, 'returnDepartureTime', text)
                                      }
                                      placeholder="08:00"
                                      keyboardType="numeric"
                                    />
